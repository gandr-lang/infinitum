...
| Operational log | `oplog`, `pretty` | ninfer's stderr lines, number formats included: `StartupLog`'s startup phases as ninfer writes them to a non-terminal stderr (`starting engine`, `loading weights`, ten-second progress with rate and ETA, `weights ready`, the host-state and host-KV pins, `CUDA graphs ready`, one `startup failed`), the startup capacity and context-cache lines, `warmup complete` for a warm-up of a quarter second or more, `listening on`, one `req#<n> started` and one `done` or `failed` line per request, and a `throughput` line each stats interval that saw activity. |
