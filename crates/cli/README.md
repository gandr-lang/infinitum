...
`serve` plans the round as `generate` does, then opens the Engine with the chat template (the artifact's own when `--chat-template` is absent), logs its resolved capacities, runs one short warm-up request, and prints `listening on <address> as <model id>`; operational lines go to stderr in ninfer's words. It answers `GET /health`, `GET /v1/models` and `POST /v1/chat/completions`, streamed or not, with tools, the reasoning channel and `chat_template_kwargs`, through [`infinitum-serve`](../serve/README.md). `--api-key` requires that key as a bearer token or `x-api-key` on every route but `/health`, and an empty key leaves the API open, as ninfer's does; `--model-id` names the model clients ask for, the artifact's model name by default. `--max-context` (default 8192) is the per-request ceiling; `--kv-capacity` (default the context ceiling) sizes the Main KV cache shared by every request, at least `--max-context`, or from device memory with `auto`; `--kv-dtype` (`bf16`, `int8`, `fp8`, `nvfp4` or `k8v4`, default `bf16`) stores it; `--prefill-chunk` (default 1024, a positive multiple of 128) sets the prompt tokens per prefill step; `--max-concurrency` (default 1, at most 8) is how many requests run at once, each on its own Engine lane, with the rest waiting for a lane; `--device-state-slots` (default one per lane), `--host-state-slots` (default 8) and `--host-kv-mib` (default 8192) size the context cache that lets a later request reuse an earlier one's prefix; `--rope-scaling-factor` (default 1, at most 16) stretches RoPE positions past `--rope-scaling-original-context` (default 262144) so `--max-context` can exceed the model's native window; `--default-max-tokens` (default 8192) is the output limit of a request that names none, which the Engine clamps to the context left; `--default-thinking-budget` (absent by default) caps thinking for a request that does not turn it off, its control tokens counting toward the output limit; `--pending-timeout-ms` (default 30000) bounds how long a request waits for admission; and `--max-request-mib` (default 384) caps the request body, refusing a larger one with 413 `request_too_large`; `--log-stats-interval-ms` (default 5000, 0 off) is how often a throughput line is logged for an interval that saw activity. `--cuda-graph` defaults to `on`, and `--host`/`--port` default to `127.0.0.1:8080`. A client that disconnects cancels its request.
