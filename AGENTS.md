...
| `ci-local.md` container image | Adopted whole: `.github/workflows/ci-image.yml` publishes `ghcr.io/silvanshade-org/infinitum-ci` under the pin-file tag, and `INFINITUM_CI_IMAGE` — a repository variable — relays the switch through `ci-image-ref` into each Rust job's `container`. Empty is the bootstrap state until the first image build is green and its measurement is recorded. | A measured rollback empties the switch permanently. |
