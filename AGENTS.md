...
| `ci-local.md` container image | Adopted whole: `.github/workflows/ci-image.yml` publishes `ghcr.io/silvanshade-org/infinitum-ci` under the pin-file tag, and `INFINITUM_CI_IMAGE` — a repository variable — relays the switch through `ci-image-ref` into each Rust job's `container`. The image bakes the toolchain, pinned CI tools, and its Dylint driver; the bare-runner path installs them and restores exact compiler- and revision-matched Dylint artifacts. A pin-file change publishes a new tag before the variable switches. | A measured rollback empties the switch permanently. |
