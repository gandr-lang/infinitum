...
| ----------- | ------------ | -------- |
| `rust.md` crate shape, the `<category>-<name>` directory schema and its package prefix | A workspace whose members are `crates/*`. Two members: `crates/cli`, the published-name holder, whose package is the reserved name `infinitum` rather than a derivation of its directory — the page's own exception for that case; and `crates/changelog`, package `infinitum-changelog`, `publish = false`, the workflow binary the `changelog` task launches. The category axis stays unspent: both members are named for what they are, and a category would carry one value each. | A third member shares a category with one of these. |
...
| `rust.md` publication | This crate publishes to crates.io; the placeholder `0.0.0` is the first release, so `publish` is `true` rather than the page's default `false`. Publishing stays manual and the owner's: no CI lane and no agent runs `cargo publish`. | None. |
